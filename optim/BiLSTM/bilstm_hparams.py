...
def define_model_BiLSTM(adapted_text_vectorization_layer, model_params: dict, label_count: int):
    ...
    x = tf.keras.layers.Bidirectional(
        layer=tf.keras.layers.LSTM(units=model_params['lstm_units']), name='BiLSTM')(x)
    x = tf.keras.layers.Dense(units=model_params['dense_1_size'], activation='relu', name='Dense_1')(x)
...
'''@nni.variable(nni.quniform(25, 200, 25), name=embedding_dimension)'''
...
with mlflow.start_run(experiment_id=eid, nested=False, tags={'master':True} ) as master_run:
    ...
    for i in range(0,3):
        ...
        with mlflow.start_run(experiment_id=eid,
                              run_name= f"{master_run.data.tags['mlflow.runName']}-{i}", nested=True, tags={'master':False}) as run:
            mlflow.log_params(params=model_params)
    ...
    mlflow.log_dict(master_results_, 'iterations_mean.txt')
