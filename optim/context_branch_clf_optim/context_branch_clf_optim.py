...
with mlflow.start_run(experiment_id=eid, nested=False, tags={'master': True}) as master_run:
    ...
    for idx in range(len(train_subsets)):
        ...
        with mlflow.start_run(experiment_id=eid,
            ...
            model = ContextBranchSentenceClassifier(
            ...
            model.fit(x=(model.train_texts[:, 0], model.train_texts[:, 1], model.train_texts[:, 2]),
                      y=model.train_labels,
                      batch_size=model_params['batch_size'],
                      epochs=model_params['max_epochs'],
                      validation_data=((model.validation_texts[:, 0],
                                        model.validation_texts[:, 1],
                                        model.validation_texts[:, 2]),
