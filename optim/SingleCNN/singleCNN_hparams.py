...
def define_model_SingleCNN(adapted_text_vectorization_layer, model_params: dict, label_count: int):
    ...
    x = tf.keras.layers.MaxPooling1D(
        pool_size=model_params['maxpool_1_size'])(x)
    x = tf.keras.layers.Flatten()(x)
    x = tf.keras.layers.Dropout(
        rate = model_params['drop_0_rate'],
        name='drop_0')(x)
