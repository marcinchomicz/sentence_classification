...
ENAME = 'ParamSearch_BiLSTM-CNN-2'
...
def define_model_LSTM_CNN_2(adapted_text_vectorization_layer, model_params: dict, label_count: int):
    ...
    x = tf.keras.layers.Bidirectional(
        layer=tf.keras.layers.LSTM(
            units=model_params['bilstm_0_size'],
            return_sequences=True),
        name='BiLSTM_0')(x)
    x1 = tf.keras.layers.Bidirectional(
        layer=tf.keras.layers.LSTM(
            units=model_params['bilstm_1_size'],
            return_sequences=False),
        name='BiLSTM_1')(x)
    x1 = tf.keras.layers.Dropout(
        rate=model_params['drop_1_rate'],
        name='Drop_1')(x1)
    x1 = tf.keras.layers.Dense(
        units=model_params['dense_0_size'],
        activation=model_params['dense_0_activation'],
        name='Dense_0')(x1)
    x2 = tf.keras.layers.Convolution1D(
        ...
        activation='relu',
        name='Conv1D')(x)
    x2 = tf.keras.layers.MaxPooling1D(
        pool_size=model_params['pool1d_pool_size'],
        name='MaxPool_1')(x2)
    x2 = tf.keras.layers.Flatten(
        name='Flatten')(x2)
    x2 = tf.keras.layers.Dropout(
        rate=model_params['drop_2_rate'],
        name='Drop_2')(x2)
    ...
    x = tf.keras.layers.Dense(
        units=model_params['dense_2_size'],
        activation=model_params['dense_2_activation'],
        name='Dense_2')(x)
...
'''@nni.variable(nni.choice(16, 32, 48, 64, 96, 128), name=bilstm_0_size)'''
bilstm_0_size = 128
'''@nni.variable(nni.choice(16, 32, 48, 64, 96, 128), name=bilstm_1_size)'''
bilstm_1_size = 128
'''@nni.variable(nni.uniform(0., 0.5), name=drop_1_rate)'''
drop_1_rate = 0.
'''@nni.variable(nni.quniform(8, 128, 8), name=dense_0_size)'''
dense_0_size = 64
'''@nni.variable(nni.choice("elu", "relu", "gelu", "selu"), name=dense_0_activation)'''
...
'''@nni.variable(nni.uniform(0., 0.5), name=drop_2_rate)'''
drop_2_rate = 0.
...
'''@nni.variable(nni.quniform(8, 128, 8), name=dense_2_size)'''
dense_2_size = 64
'''@nni.variable(nni.choice("elu", "relu", "gelu", "selu"), name=dense_2_activation)'''
...
mp_LSTM_CNN_2 = {
    ...
    'bilstm_0_size':int(bilstm_0_size),
    'bilstm_1_size':int(bilstm_1_size),
    'drop_1_rate':drop_1_rate,
    'dense_0_size':int(dense_0_size),
    'dense_0_activation': dense_0_activation,
    'conv1d_size':int(conv1d_size),
    'conv1d_kernel_size':int(conv1d_kernel_size),
    'pool1d_pool_size':2,
    'drop_2_rate':drop_2_rate,
    'dense_1_size':int(dense_1_size),
    'dense_1_activation':dense_1_activation,
    'dense_2_size':int(dense_2_size),
...
model_params = mp_LSTM_CNN_2
model_definition = define_model_LSTM_CNN_2
...
with mlflow.start_run(experiment_id=eid, nested=False, tags={'master': True}) as master_run:
    ...
    for i in range(0, 3):
        ...
        with mlflow.start_run(experiment_id=eid,
            ...
            mlflow.log_text(inspect.getsource(model_definition), 'model_building_method.txt')
            mlflow.log_dict(model_params, 'model_params.txt')
            mlflow.log_dict({'cuda_device': os.environ['CUDA_VISIBLE_DEVICES']}, 'environment_settings.txt')
